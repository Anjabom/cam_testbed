...
#   ★이 기계의 mp4 를 코덱 상관없이 연다★ — 서버가 프레임만 넘긴다
```
...
`tb.run` 밖에 하나 더: `python3 -m tb.encode <파일|폴더>` — mp4v 영상을 스튜디오가 여는
H.264 로 바꾼다(§4.2.1).
...
★왜 서버가 있나★ 브라우저는 `mp4v`(cv2 기본 코덱)를 열지 못한다. 이 기계의 녹화가
전부 그 코덱이라, 서버 없는 정적 페이지로는 **가진 영상을 하나도 못 여는** 도구가 됐다.
실측: http 로 그대로 내려 줘도 `<video>` 는 `error code 4` — 파일을 어디서 주느냐가
아니라 브라우저 디코더의 한계다.

서버는 그 벽을 우회한다: **cv2 로 디코드해 프레임 한 장만 넘긴다.** 브라우저가 영상을
만지지 않으므로 코덱이 무엇이든 상관없어진다.

| | 실측 (2.2GB `mp4v`) |
|---|---|
| 임의 프레임 | 탐색 13ms + 디코드 3ms + JPEG 5ms = **21ms** |
| 이어지는 프레임 | **1ms** (탐색을 건너뛴다) |

★그림은 여전히 브라우저가 그린다★ 예전 웹앱은 BEV 까지 서버가 그려 합성 JPEG 을
내려보냈고, 조작마다 71ms 씩 왕복했다. 지금은 원본 프레임만 받고 왜곡보정·BEV·오버레이는
GPU 에서 하므로 **드래그가 즉시** 반응한다.

★서버가 하지 않는 것★ — 쓰기 없음(내보내기는 브라우저가 내려받는다) · 실행 없음 ·
업로드 없음 · 인증 없음(`127.0.0.1` 고정이라 밖에서 들어올 길이 없다). 읽는 범위는
...
```bash
python3 tools/bake_reference.py     # cv2 가 낸 정답표를 web/reference.js 로 굽는다
...
그림을 그리는 셰이더(`web/render.js`)까지 같은지는 따로 잰다:
...
### 4.2 무엇을 맞추나 — `web/tuning.js` 한 곳이 정한다
...
**끌기와 숫자, 두 길이 같은 값을 본다.** 대상을 고르면 오른쪽에 좌표 칸이 나온다 —
사각형은 `TL/TR/BR/BL` 의 x·y, ROI 는 좌상·우하의 x·y, 척도는 값 자체. 끌면 숫자가
따라 움직이고 숫자를 치면 그림이 따라 움직인다(타이핑 중인 칸은 건드리지 않는다).
1px 이 뜻을 갖는 자리가 있어서 ★끌기만으로는 부족하다★ — 옛 웹앱에도 있던 길이다.
...
사유와 고칠 명령을 띠로 띄운다. 고치는 길은 둘:

```bash
python3 -m tb.encode <파일 또는 폴더>       # <이름>__web.mp4 를 옆에 만든다
```

NVENC 이 있으면 실시간의 **14배**로 돈다(10분짜리 2.2GB 영상 ≈ 40초). 없으면 libx264 로
내려간다. `--cpu` 로 강제할 수 있다.

애초에 그 파일이 나오게 하려면 **녹화 쪽**에서 두 개를 남긴다 — `cam_record` 의
`web_copy`(기본 켬)가 녹화 후 `<이름>__web.mp4` 를 만든다. 원본은 그대로 두고 시험이 쓴다.

★변환본으로 시험하지 않는다★ — 시뮬레이터는 cv2 로 읽으니 `mp4v` 도 잘 돈다. 재인코딩된
화질로 재면 실차가 보는 것과 다른 것을 재게 된다(경계 ⑥).
...
1. `tb/*.py` 와 `web/*` 에 **대상 워크스페이스의 이름을 한 글자도 쓰지 않는다**
   (이름은 계약과 `web/tuning.js` 라는 **데이터**에만 있다)
