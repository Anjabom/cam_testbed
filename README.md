...
| **새 시험 시작** | **새로 찍은 영상으로 갈아탄다** — 영상 등록 → 시나리오(본 떠서) → 카메라 맞추기 → 돌리기. 순서를 화면이 들고 있다 (§13.9) |
...
**«새 시험 시작»** 탭이 순서를 들고 있다. 화면 네 곳을 거쳐야 하는 일이라
...
| 2 시나리오 만들기 | `stopline_field.yaml` 을 **본으로 떠서** 영상만 갈아 끼운다 — 판정 20여 개와 **그 근거 주석까지 그대로** 물려받는다 | 새 이름. 구간은 비워 둔다(전체) |
...
> 카메라로 찍힌 것이 아니면 실차에 그대로 쓰면 안 된다. 그래서 «새 시험 시작» 이
