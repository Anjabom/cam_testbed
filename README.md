...
원칙 하나만 알면 헤맬 일이 없다 — **파일을 쓰는 일은 «새 시험 시작», 돌리는 일은
...
| **새 시험 시작** | **파일을 건드리는 일은 전부 여기.** 워크스페이스(계약) → 영상 → 시나리오(본 떠서 / 빈 틀) → 카메라 맞추기 → 돌리기. 순서를 화면이 들고 있다 (§13.9) |
...
| **실행** | **고르고 돌리기만 한다**(등록은 «새 시험 시작»). 시나리오를 고르면 **그게 실제로 쓸 계약·워크스페이스·영상 파일·구간·파라미터를 풀어서** 보여 준다. ⛔ 가 있으면 버튼이 잠기고, **소스가 빌드보다 새로우면** ⚠ 가 붙는다 — 그 자리에서 «워크스페이스 빌드» 를 누르면 된다. 빌드/점검/주입검증/실행/중지 · 진행률·라이브 화면·로그 |
...
**«새 시험 시작»** 탭이 순서를 들고 있다. 화면 여러 곳을 거쳐야 하는 일이라
...
| 2 시나리오 만들기 | **본 떠서** = `stopline_field.yaml` 의 영상만 갈아 끼운다 — 판정 20여 개와 **그 근거 주석까지 그대로** 물려받는다. **빈 틀** = 새 워크스페이스용 | 새 이름. 구간은 비워 둔다(전체) |
