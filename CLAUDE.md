...
1. **`tb/*.py` 와 `docs/*` 에 대상 워크스페이스의 토픽명·필드 배치·노드명·파라미터명을 한 글자도
   쓰지 않는다.** 시험 쪽은 `contracts/*.yaml` 에만, 스튜디오 쪽은 `docs/tuning.js`(데이터)에만
...
3. **기하는 두 벌이지만 ★대조로 묶여 있다★.** [2026-09-06 개편] 스튜디오가 정적 페이지가
   되면서 서버가 없어졌고, 기하가 `docs/geom.js` 에 한 벌 더 생겼다. 예전 규칙(「JS 에 한 벌
   ...
   **`docs/geom.js` 나 `tb/geometry.py` 를 고치면 반드시 `python3 -m tb.selftest` 를 돌린다.**
   그림을 그리는 `docs/render.js` 의 셰이더도 같은 식이어야 한다 — `tools/shader_check.html`.
4. **화면의 편집 대상은 `docs/tuning.js` 가 정한다** — 무엇을 맞출지와 그것이 어느 노드의
...
5. **스튜디오는 아무것도 실행하지 않는다.** 서버가 없다 — 정적 파일뿐이고, 영상은 여는
   사람의 브라우저 안에서만 열린다. **서버를 다시 들이지 않는다**: 그 순간 인증·업로드·
   폴더 훑기·방화벽이 통째로 딸려 오고, 그 전부가 예전에 실제로 있던 짐이다.
   노드를 봐야 하는 일(워크스페이스 값 읽기 · BEV 대조)은 CLI 에 있다
   (`tb.run params` · `tb.run verify`).
...
서브커맨드: `doctor · run · replay · reanalyze · diff · export · params · build · verify · list`

### ② 카메라 보정 — `docs/` 정적 페이지 (★서버가 없다★)

```
https://anjabom.github.io/cam_testbed/     # 어느 기기에서든 링크만 있으면 열린다
docs/index.html                            # 클론했으면 그냥 파일로 열어도 같다
```

설치도 로그인도 포트도 없다. 영상은 **여는 사람의 브라우저 안에서만** 열리고 어디에도
올라가지 않는다 — 그래서 예전에 있던 토큰·업로드·`browse_roots`·systemd·방화벽이
...
`docs/` 는 그대로 GitHub Pages 로 나간다(`main` 브랜치의 `/docs`). **공개된다는 뜻이므로
실측값·경로·영상을 그 폴더에 두지 않는다.**
...
- 스튜디오는 **외부 의존성 0**(CDN·웹폰트·JS 라이브러리 없음). 대회 현장에서 네트워크 없이
  `docs/index.html` 을 파일로 열어도 돌아야 한다 — `fetch` 로 데이터를 읽지 않고 전부
  `<script>` 로 싣는 이유다. 의존성을 추가하지 않는다.
