 ...
 * [2026-09-06] 스튜디오가 정적 페이지로 옮겨 오면서 계약을 읽을 서버가 없어졌다.
 * 그래서 계약 4개(white_camera · white_vote · white1_stopline · black_vote)의
 * 대상들을 ★합쳐서 여기 한 벌★ 로 두었다. 같은 것을 다른 이름으로 부르던
 * 자리(ipm_src_pts ↔ bev_src_pts, pixel_to_meter_bev ↔ bev_px_to_m)는
 * 한 항목이 이름 여러 개를 들고 내보내는 것으로 합쳤다.
 *
 * ★기본값은 자리표시자다★ 실측 캘리브 값을 여기 넣지 않는다 — 저장소가 공개다.
 * 자기 값은 «불러오기» 로 JSON 을 열어 쓴다(«내보내기» 가 그 형식으로 준다).
 *
...
window.TUNING = {
  //  ── 카메라 ─────────────────────────────────────────────────────
  //  size/K/D 는 노드가 소스에 박아 둔 어안 보정 모델과 같아야 한다.
  //  D 가 전부 0 이면 「보정하지 않음」이고, 그때 K 는 그림에 영향을 주지 않는다.
  camera: {
    size: [1920, 1080],
    K: [960.0, 960.0, 960.0, 540.0],       // fx fy cx cy — 자리표시자(화면 중앙·화각 대충)
